...
if($logged){
$getAccountCoins = $db->query('SELECT `id`, `premdays`, `premium_points`' . 'FROM `accounts`' . 'WHERE `id` = ' . $account_logged->getId() .'');
$getAccountCoins = $getAccountCoins->fetch();
?>
<div class="CharacterTradeTibiaCoinBalance"><?php echo $getAccountCoins['premium_points'] ?><img src="<?php echo $template_path; ?>/images//account/icon-tibiacoin.png" class="VSCCoinImages" title="Tibia Coins"> <?php echo $getAccountCoins['premium_points'] ?><img src="<?php echo $template_path; ?>/images//account/icon-tibiacointrusted.png" class="VSCCoinImages" title="Transferable Tibia Coins"></div>
<?php
}
...
if($getCharacter['level'] >= 8){
	$verif_level = '<img src="/images/premiumfeatures/icon_yes.png">';
	$next_truecount++;
}else{
	$verif_level = '<img src="/images/premiumfeatures/icon_no.png">';
}
...
if($frags_count == 0){
	$verif_frags = '<img src="/images/premiumfeatures/icon_yes.png">';
	$next_truecount++;
}else{
	$verif_frags = '<img src="/images/premiumfeatures/icon_no.png">';
}
/* GET FRAGS PLAYERS END */


/* GET COINS */
$getCoins = $db->query('SELECT `premium_points`' . 'FROM `accounts`' . 'WHERE `id` = ' . $getCharacter['account_id'] .'');
$getCoins = $getCoins->fetch();

if($getCoins >= 10) {
	$verif_coins = '<img src="/images/premiumfeatures/icon_yes.png">';
	$next_truecount++;
}else{
	$verif_coins = '<img src="/images/premiumfeatures/icon_no.png">';
}
/* GET COINS END */
...
if($getHouse == 0) {
	$verif_house = '<img src="/images/premiumfeatures/icon_yes.png">';
	$next_truecount++;
}else{
	$verif_house = '<img src="/images/premiumfeatures/icon_no.png">';
}
/* GET HOUSE END */
...
if($getGuildOwner == 0 and $getGuildInvited == 0 and $getGuildMember == 0) {
	$verif_guild = '<img src="/images/premiumfeatures/icon_yes.png">';
	$next_truecount++;
}else{
	$verif_guild = '<img src="/images/premiumfeatures/icon_no.png">';
}
/* GET GUILD END */
...
if($getOnline == 0){
	$verif_online = '<img src="/images/premiumfeatures/icon_yes.png">';
	$next_truecount++;
}else{
	$verif_online = '<img src="/images/premiumfeatures/icon_no.png">';
}
/* GET CHARACTER ONLINE */
...
/* ACCOUNT BY PLAYER */
$getAccount = $db->query('SELECT `id`, `premdays`, `premium_points`' . 'FROM `accounts`' . 'WHERE `id` = ' . $getCharacter['account_id'] .'');
...
$getAccountLogged = $db->query('SELECT `id`, `premdays`, `premium_points`' . 'FROM `accounts`' . 'WHERE `id` = ' . $account_logged->getId() .'');
				...
				<tr>
                  <td><div class="TableContentContainer">
                      <table class="TableContent" style="border:1px solid #faf0d7;" width="100%">
                        <tbody>
                          <tr>
                            <td style="font-weight:normal;"><?php echo $getAccount['premium_points'] ?> <img src="<?php echo $template_path; ?>/images/account/icon-tibiacoin.png"> (<?php echo $getAccount['premium_points'] ?> <img src="<?php echo $template_path; ?>/images/account/icon-tibiacointrusted.png">)</td>
...
$getAccount = $db->query('SELECT `id`, `premdays`, `premium_points`' . 'FROM `accounts`' . 'WHERE `id` = ' . $account_logged->getId() .'');
$getAccount = $getAccount->fetch();
...
$getCoinsAccountLogged = $db->query('SELECT `id`, `premium_points`' . 'FROM `accounts`' . 'WHERE `id` = ' . $account_logged->getId() .'');
$getCoinsAccountLogged = $getCoinsAccountLogged->fetch();

$charbazaar_mycoins = $getCoinsAccountLogged['premium_points'];
$charbazaar_create = $config['bazaar_create'];
$charbazaar_mycoins_calc = $charbazaar_mycoins - $charbazaar_create;

if($getCoinsAccountLogged['premium_points'] > $charbazaar_create){

$update_accountcoins = $db->exec('UPDATE `accounts` SET `premium_points` = '.$charbazaar_mycoins_calc.' WHERE `id` = '.$getAccount['id'].'');

$SQL->query('INSERT INTO `myaac_charbazaar` (`account_old`, `account_new`, `player_id`, `price`, `date_end`, `date_start`) VALUES ('.$account_old.', '.$account_new .', '.$player_id.', '.$price.', '.$date_end.', '.$date_start.')');
